...
class BusinessCase:
    def __init__(self, fine_good:float, fine_toxic:float) -> None:
        self.__fine_good = fine_good
        self.__fine_toxic = fine_toxic

    def get_profit(self, matrix:ConfusionMatrix) ->float:
        return matrix.FN * self.__fine_good + matrix.FP * self.__fine_toxic

class Experiment:
    __metric = None

    def __init__(self, name:str, y_test:np.array, y_pred:np.array, business_case:BusinessCase):
        self.__name = name
        self.__y_test = y_test
        self.__y_pred = y_pred
        self.__business_case = business_case
    ...
    def get_profit(self, thresholds:float= None) ->float:
        if thresholds is None:
            if self.__metric is None:
                self.calc_metric()
            cnf_matrix = self.__metric.Confusion_matrix
        else:
            cnf_matrix = self.__get_confusion_matrix(thresholds)
        return self.__business_case.get_profit(cnf_matrix)
    ...
    def show_profit_calibration_plots(self):
        ...
        min_toxic_pay = None
        min_toxic_threshold = None
        for threshold in thresholds:
            toxic_pay = self.get_profit(threshold)
            if min_toxic_pay is None or min_toxic_pay>toxic_pay:
                min_toxic_pay = toxic_pay
                min_toxic_threshold = threshold
            toxic_paies.append(toxic_pay)
            
        plt.plot(thresholds,toxic_paies)
        plt.ylabel('Расходы руб.')
        plt.xlabel('Порог')
        plt.title('Зависимость расходов от уровня класификации')

        plt.plot(min_toxic_threshold, min_toxic_pay, c='r', marker='o')
        plt.plot([min_toxic_threshold, min_toxic_threshold], [0.0,min_toxic_pay ], 'r--')
        plt.plot([0.0, min_toxic_threshold], [min_toxic_pay, min_toxic_pay ], 'r--')
        plt.annotate('Порог: %0.3f, Расход: %0.0f' %(min_toxic_threshold, min_toxic_pay), 
...
class MetricRegressionManager:
    ...
    def __init__(self, fine_good:float, fine_toxic:float,):
    ...
    def apply(self, name:str, y_test: np.array, y_pred: np.array):
        self.__repo.append(name, Experiment(name, y_test, y_pred, self.__business_case))
